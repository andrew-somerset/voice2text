...
- **pywinauto UIA backend** — used narrowly to retain an opaque focused element and call only `set_focus()` before paste. Never read UIA names, values, text, automation IDs, or surrounding content.
...
- Attach an opaque UIA focused-element wrapper to the target where available. On the transcription worker, initialize COM, call only `set_focus()`, then synthesize a balanced Ctrl+V with `SendInput`. Fall back to pure Win32 menu cancellation and exact-control restoration when UIA is unavailable. Never read element metadata or content.
- Do not use cross-process `WM_PASTE` as a success path: Windows 11 Notepad returned success while inserting nothing. A paste outcome must identify only a delivery attempt, and blocked focus/input must surface the explicit-copy fallback.
...
- CI-safe tests: gesture timing, event routing, recorder buffer and meter logic, Mac-style pill bar targets, test-route cancellation and zeroing, local worker queues, single-instance behavior, detached ready/stop lifecycle, startup registry dispatch, opaque UIA target redaction/focus, balanced SendInput, generation-safe clipboard restoration, transcript filtering, OAuth PKCE utilities, mocked Glean parsing, config validation, and model checksum validation.
...
- Windows integration tests: Raw Input emits only configured-trigger transitions plus identity-free chord markers; production UIA focus restoration plus balanced SendInput inserts an exact marker in disposable Windows 11 Notepad; the clipboard is restored; DPAPI round-trips only for the current user.
